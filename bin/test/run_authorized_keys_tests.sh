...
cat $tmpdir/unittest.pem $tmpdir/intermediate.pem > $tmpdir/chain.pem

intermediate_fingerprint=$(openssl x509 -noout -fingerprint -sha1 -inform pem -in $tmpdir/intermediate.pem | sed -n 's/SHA1 Fingerprint=\(.*\)/\1/p')
unittest_fingerprint=$(openssl x509 -noout -fingerprint -sha1 -inform pem -in $tmpdir/unittest.pem | sed -n 's/SHA1 Fingerprint=\(.*\)/\1/p')
...
for testfile in "${TOPDIR}"/test/input/direct/* ; do
    filename="${testfile##*/}"
    $TOPDIR/bin/test/test_authorized_keys.sh "${OPENSSL}" "${TOPDIR}/src/opt/aws/bin/parse_authorized_keys" "${tmpdir}/chain.pem" "${tmpdir}/ca.pem" "${tmpdir}" "${testfile}" "${TOPDIR}/test/expected-output/${filename}"
    if [ $? -ne 0 ] ; then
        exit_status=1
    fi
done
...
for testdir in "${TOPDIR}"/test/input/unsigned/* ; do
    ...
    if [ $? -ne 0 ] ; then
        ...
        # Run the actual test
        $TOPDIR/bin/test/test_authorized_keys.sh "${OPENSSL}" "${TOPDIR}/src/opt/aws/bin/parse_authorized_keys" "${tmpdir}/chain.pem" "${tmpdir}/ca.pem" "${tmpdir}" "${tmpdir}/${filename}" "${TOPDIR}/test/expected-output/${filename}"
        if [ $? -ne 0 ] ; then
            exit_status=1
        fi
    fi
